...
export function calcDeltaB1(data: TableData[], w13: number, deltaB1: number) {
    ...
    data.map((item) => {
        sum += -2 * (item.c1Normalize - item.c1calcNorm) * w13 * (Math.exp(item.x1) / (1 + Math.exp(item.x1)));
    });
...
export function calcDeltaB2(data: TableData[], w23: number, deltaB2: number) {
    ...
    data.map((item) => {
        sum += -2 * (item.c1Normalize - item.c1calcNorm) * w23 * (Math.exp(item.x2) / (1 + Math.exp(item.x2)));
    });
...
export function calcDeltaW13(data: TableData[], deltaW13: number) {
    ...
    data.map((item) => {
        sum += -2 * (item.c1Normalize - item.c1calcNorm) * item.y1;
    });
...
export function calcDeltaW23(data: TableData[], deltaW23: number) {
    ...
    data.map((item) => {
        sum += -2 * (item.c1Normalize - item.c1calcNorm) * item.y2;
    });
